...
export class LayoutComponent implements OnInit {
  ...
  constructor(
    ...
    iconRegistry.addSvgIconLiteral(
      'exit',
      sanitizer.bypassSecurityTrustHtml(EXIT_ICON)
    );
  }
  ...
  ngOnInit() {
    this.route.queryParamMap.subscribe((params) => {
      ...
      if (folder) {
        ...
        this.currentFolder = folder.toUpperCase();
      } else {
  ...
  onActivate(component: any) {
    if (component instanceof MessageListComponent) {
      ...
      // Subscribe to mail message mode changes
      this.messageListComponent.mailMessageModeChange.subscribe((mode) => {
        this.isComposing = mode.isComposing;
        this.isReplyMode = mode.isReplyMode;
        this.isForwardMode = mode.isForwardMode;
      });
    }
  }
  ...
  saveMessage() {
    // Archive the current message and navigate back to list
    if (this.messageListComponent) {
      const messageId = this.messageListComponent.currentMsgId;
      if (messageId) {
        // Clear any existing selection and select only the current message
        this.messageListComponent.selectedMessages.clear();
        this.messageListComponent.selectedMessages.add(messageId);

        // Archive the selected message (which is the current message)
        this.messageListComponent.archiveSelectedMessages();

        // Close the message view to return to the list
